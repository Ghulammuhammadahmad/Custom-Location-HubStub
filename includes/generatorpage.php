...
// Labels that should NOT be replaced (kept as-is)
define('CLHS_EXCLUDED_LINK_LABELS', array(
...
// Replace link labels with URLs using URL_RESOLUTION_TABLE from AI response, excluding specified labels
function clhs_replace_link_labels($data, $url_resolution_table) {
    // Make sure we have a valid array
    if (!is_array($url_resolution_table) || empty($url_resolution_table)) {
        return $data;
    }
    
    // Filter out excluded labels from the replacement map
    $excluded_labels = CLHS_EXCLUDED_LINK_LABELS;
    $link_map = array_diff_key($url_resolution_table, array_flip($excluded_labels));
    ...
    if (is_array($data)) {
        ...
        foreach ($data as $key => $value) {
            ...
            $result[$key] = clhs_replace_link_labels($value, $url_resolution_table);
        }
        return $result;
...
function get_json_from_openai($acf_schema, $user_input = null, $extra_instructions = '') {
    ...
    // Add URL_RESOLUTION_TABLE to the schema (AI will populate this with LINK_LABEL => URL mappings)
    $schema['properties']['URL_RESOLUTION_TABLE'] = [
        "type" => "object",
        "description" => "URL Resolution Table mapping [LINK_LABEL] patterns to their corresponding URLs. Include mappings for: [LINK_ALL_LOCATIONS], [LINK_AREAS_WE_SERVE], [LINK_APPOINTMENT_FORM], [LINK_DIRECTIONS_PAGE], [LINK_REVIEWS_PAGE], [LINK_TEAM_PAGE], [LINK_RESEARCH_CENTER]. Do NOT include map links like [LINK_FARMERS_BRANCH_MAP], [LINK_FORT_WORTH_MAP], etc.",
        "additionalProperties" => [
            "type" => "string",
            "description" => "URL for the corresponding LINK_LABEL"
        ]
    ];

    if (isset($schema['properties']) && is_array($schema['properties'])) {
        $all_keys = array_keys($schema['properties']);
        // Exclude URL_RESOLUTION_TABLE from required fields (it's generated by AI but not saved as a field)
        $required_keys = array_filter($all_keys, function($key) {
            return $key !== 'URL_RESOLUTION_TABLE';
        });
        $schema['required'] = array_values($required_keys); // require all except URL_RESOLUTION_TABLE
    } else {
...
// flush();
    ...
    // Extract URL_RESOLUTION_TABLE from AI response and use it to replace link labels
    $url_resolution_table = array();
    if (!empty($json_obj['URL_RESOLUTION_TABLE']) && is_array($json_obj['URL_RESOLUTION_TABLE'])) {
        $url_resolution_table = $json_obj['URL_RESOLUTION_TABLE'];
    }
    
    // Replace LINK labels with URLs using the URL_RESOLUTION_TABLE from AI response
    if (!empty($url_resolution_table)) {
        $json_obj = clhs_replace_link_labels($json_obj, $url_resolution_table);
    }
...
function generate_pagefrom_openairesponse($aijsonresult, $pagename) {
    ...
    // Exclude special fields: meta_title, meta_description, slug, stub_title, URL_RESOLUTION_TABLE
    $special_fields = array('meta_title', 'meta_description', 'slug', 'stub_title', 'URL_RESOLUTION_TABLE');
